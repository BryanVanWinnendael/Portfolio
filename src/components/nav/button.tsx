...
const Button = ({ isActive, toggleMenu }: any) => {
  ...
  const secondary_bg =
    background_color === "bg-primary" ? "bg-secondary" : "bg-primary"
  const text_color_menu =
    background_color === "bg-primary" ? "text-secondary" : "text-primary"
  const text_color_close =
    background_color === "bg-primary" ? "text-primary" : "text-secondary"
...
const PerspectiveText = ({
  ...
  return (
    <div className="flex flex-col justify-center items-center h-full w-full transform-style:preserve-3d transition-transform duration-750 ease-in-out cubic-bezier(0.76, 0, 0.24, 1)">
      <p
        className={`${text_color} font-semibold transition-all duration-750 ease-in-out cubic-bezier(0.76, 0, 0.24, 1) pointer-events-none uppercase`}
      >
        {label}
      </p>
      <p className="absolute font-semibold transform-origin-bottom-center transform -rotate-x-90 -translate-y-9 opacity-0">
        {label}
      </p>
    </div>
  )
